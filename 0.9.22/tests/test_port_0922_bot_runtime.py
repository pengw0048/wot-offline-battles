...
def _slope_plane_probe(slope_z):
...
class BotRuntimeTests(unittest.TestCase):
    ...
    def test_uphill_plane_pitches_the_bot_nose_up(self):
        pitch, roll = self.module.slope_pose(
            _slope_plane_probe(0.2), (0.0, 0.0, 0.0),
            0.0, 3.5, 1.7)
        expected = -math.atan2(0.2 * 7.0, 7.0) * 0.9 * 0.5
        self.assertAlmostEqual(expected, pitch)
        self.assertAlmostEqual(0.0, roll)

    def test_bot_slope_pose_smoothing_converges(self):
        target = -math.atan2(0.2 * 7.0, 7.0) * 0.9
        pitch, roll = 0.0, 0.0
        for unused in range(12):
            pitch, roll = self.module.slope_pose(
                _slope_plane_probe(0.2), (0.0, 0.0, 0.0),
                0.0, 3.5, 1.7, pitch, roll)
        self.assertAlmostEqual(target, pitch, places=3)

    def test_extreme_bot_slope_pose_is_clamped(self):
        pitch, roll = self.module.slope_pose(
            _slope_plane_probe(5.0), (0.0, 0.0, 0.0),
            0.0, 3.5, 1.7, last_pitch=-0.61, last_roll=0.0)
        self.assertLessEqual(
            math.sqrt(pitch * pitch + roll * roll), 0.61 + 1.0e-9)

    def test_missing_bot_slope_sample_keeps_last_pose(self):
        pitch, roll = self.module.slope_pose(
            lambda x, z, hint: None, (0.0, 0.0, 0.0),
            0.0, 3.5, 1.7, last_pitch=-0.3, last_roll=0.1)
        self.assertEqual((-0.3, 0.1), (pitch, roll))

    def test_side_slope_rolls_bot_without_pitch(self):
        ...
        self.assertAlmostEqual(
            math.atan2(0.2 * 3.4, 3.4) * 0.9 * 0.5, roll)
    ...
    def test_mid_reload_progress_survives_authority_manifest_restore(self):
        self.runtime.battle_start(self.start)
    ...
    def test_manifest_rejects_route_above_protocol_limit(self):
        ...
        with self.assertRaisesRegex(ValueError, '32-waypoint'):
            self.runtime._manifest_entry(state)

    def test_route_strategy_metadata_survives_worker_manifest_projection(self):
        self.runtime.battle_start(self.start)
        state = dict(self.runtime.states[11])
        state['route'] = {
