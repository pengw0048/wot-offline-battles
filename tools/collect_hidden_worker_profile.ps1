...
try {
    ...
    # Function-level _lsprof reports are written by the hidden worker itself
    # (one .txt and one .pstats per profiled window). Their exact names carry
    # the round and window numbers, so enumerate only that fixed folder.
    $lsprofRoot = Join-Path $resolvedRoot "mods\configs\offline_lan_0922"
    $lsprofCount = 0
    ...
    if ($lsprofCount -eq 0) {
        [void]$missing.Add(@{
            name = "offline-worker-lsprof-*.txt"
            source = $lsprofRoot
            reason = "no _lsprof window report has been written yet; the first window ends about 55 s after the battle goes live"
        })
    }
