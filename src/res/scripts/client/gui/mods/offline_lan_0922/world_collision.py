...
# The two upper hull heights, paired with the prepared lane values a batched
# backend supplies for them.  The inline path prepares nothing.
_UPPER_HEIGHTS = ((1.1, None), (1.6, None))


@observed('motion.prepare')
def _prepare_sweep_batch(preparation, pos, cos_y, sin_y, pose_y,
		half_width, half_back, half_front, vel, dt, airborne, motion_yaw):
	"""Evaluate one sweep's geometry in a single batch.

	Every transcendental stays here, so the batch performs only arithmetic and
	a native backend cannot introduce a different ``sin``/``cos``.  A native
	failure returns ``None``: the caller then runs the unchanged inline path
	for this sweep, and the backend stops calling itself native.
	"""
	import math
	if motion_yaw is None:
		motion_sin, motion_cos, has_motion_yaw = 0.0, 0.0, 0.0
	else:
		motion_sin = math.sin(float(motion_yaw))
		motion_cos = math.cos(float(motion_yaw))
		has_motion_yaw = 1.0
	try:
		plan = preparation.prepare_sweep(
			float(pos.x), float(pos.y), float(pos.z), sin_y, cos_y,
			pose_y[0], pose_y[1], pose_y[2],
			half_width, half_back, half_front,
			float(vel), float(dt), 1.0 if airborne else 0.0,
			motion_sin, motion_cos, has_motion_yaw)
	except Exception as error:
		native_compute.demote(repr(error))
		return None
	native_compute.note('prepare_calls')
	native_compute.note('prepare_lanes', len(plan[3]))
	return plan
...
def _posed_ray(Math, pos, x1, z1, x2, z2, local_start, local_end,
		height, pose_y, ground_ahead=None, prepared=None):
...
def _check_horizontal_collision(spaceID, pos, yaw, vel, td=None,
	...
	try:
		...
		_preparation_backend = native_compute.preparation()
		_plan = (None if _preparation_backend is None else
			_prepare_sweep_batch(
				_preparation_backend, pos, cos_y, sin_y, pose_y,
				hw, hl_back, hl_front, vel, dt, airborne, motion_yaw))
		if _plan is not None:
			_ahead, ground_plane, _bounds, lane_segments = _plan
			minimum_x, maximum_x, minimum_z, maximum_z = _bounds
		else:
		...
		for lane in lane_segments:
			(x1, z1, x2, z2, target_len,
				...
				profile_direction, profile_look) = lane[:11]
			if _plan is None:
				...
				# Keep the footprint start, but stop pose growth at the first hull
				# edge.  This chord is a conservative witness inside the swept hull
				# volume; extrapolating pitch/roll through look-ahead can pass over
				# a real wall.
				...
				footprint_z = (pos.z - sin_y * local_end[0] +
					cos_y * local_end[1])
				_lower_prepared = None
				_upper_prepared = _UPPER_HEIGHTS
			else:
				local_start = (lane[11], lane[12])
				local_end = (lane[13], lane[14])
				pose_clamped = lane[15] != 0.0
				footprint_x = lane[16]
				footprint_z = lane[17]
				_lower_prepared = (lane[18], lane[19])
				_upper_prepared = (
					(1.1, (lane[20], lane[21])),
					(1.6, (lane[22], lane[23])))
			_ground_ahead = (
			...
			start_bot, end_bot = _posed_ray(
				Math, pos, x1, z1, x2, z2, local_start, local_end,
				0.6, pose_y, _ground_ahead, _lower_prepared)
