"""Horizontal world collision, taken from the 0.9.22 port.
...
from gui.mods.offline_battle_2312.destructibles_sensor import (
...
def _solid_contact_cleared(spaceID, segment_start, segment_end, vel, td):
    ...
    2.3.1.2 keeps a destroyed fragile/module skin solid until its hide callback.
...
def _destroy_and_recast(spaceID, segment_start, segment_end, collision,
    ...
    if not _try_destroy_solid_hit(
        ...
        # skin until 2.3.1.2's hide callback.  Only that exact pending identity may
