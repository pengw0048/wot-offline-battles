"""Battle event reporting, taken from the 0.9.22 port.
...
"""Battle-feedback helpers behind explicit 2.3.1.2 adapters.
...
hangar while the 2.3.1.2 lifecycle remains under BattleRuntime control.
"""
...
class VehicleStatePresenter(object):
    """Thin adapter for the confirmed 2.3.1.2 vehicle-state presentation path."""
    ...
    def notify_observed_by_enemy(self, value):
        """Use the stock 2.3.1.2 vehicle-state event; do not call legacy GUI APIs."""
...
class SixthSenseController(object):
    ...
    generation.  Supplying all predicates explicitly is intentional: 2.3.1.2
