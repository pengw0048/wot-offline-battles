...
the mature 0.9.52 architecture (`offline_lan_0922`) and changes only the
...
- `.../offline_battle_2312/suspension.py` — the 0.9.52 ground probes and
...
- `.../offline_battle_2312/combat_rules.py` — the 0.9.52 armour and
  damage law, copied.
...
Law copied from the 0.9.52 port, unchanged, reached through the adapters
...
python3 tools/validate_wotmod.py dist/org.peng.offline_2312_battle_0.9.5.wotmod
```
...
2.3.1.2 carries one `chassisHealth` device where the 0.9.52 law tables
...
providers instead, which is what the 0.9.52 port does for a remote
vehicle.

The native body never simulates offline, so this port owns the pose. That
matches the conclusion the mature 0.9.52 port reached on its own client,
...
Conclusion, matching the 0.9.52 port's `native_motion=False`: motion has
